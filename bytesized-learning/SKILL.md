---
name: bytesized-learning
description: "Manim CE animations with Kokoro TTS narration: 3Blue1Brown math/algo videos."
version: 3.0.0
...
Use when users request: animated explanations, math animations, concept visualizations, algorithm walkthroughs, technical explainers, 3Blue1Brown style videos, or any programmatic animation with geometric/mathematical content. Creates 3Blue1Brown-style explainer videos with Kokoro TTS narration, algorithm visualizations, equation derivations, architecture diagrams, and data stories using Manim Community Edition.
...
Run `scripts/setup.sh` to verify all dependencies. Requires: Python 3.10+, Manim Community Edition v0.21+ (`pip install manim`), LaTeX (`texlive-full` on Linux, `mactex` on macOS), ffmpeg, and Kokoro TTS. Reference docs tested against Manim CE v0.21.0. See `references/troubleshooting.md` for 0.21 compatibility gotchas (`Number` removed, `Arrow` needs `Create()`, `dash_ratio` gone, `DOWN_LEFT` → `DOWN + LEFT`, background in `setup()`).
...
| TTS | **Kokoro** (local) | Narration voiceover — `af_bella` voice |

## Kokoro TTS Integration

Uses the same Kokoro model as the `voice-bridge` skill. Model files are external (exceed 5MB limit).

```python
from kokoro_onnx import Kokoro

k = Kokoro(
    "/Users/manjunathkanavi/.hermes/skills/voice-bridge/assets/kokoro/model.onnx",
    "/Users/manjunathkanavi/.hermes/skills/voice-bridge/assets/voices-v1.0.bin"
)

# Generate audio from text — use k.create(), NOT k.generate()
audio, sr = k.create("Hello world", voice="af_bella", speed=1.0)

# For custom voice style, pass the embedding as the voice parameter:
voice_emb = k.voices["af_bella"]
style = voice_emb.mean(axis=0).astype(np.float32)  # shape: (1, 256)
audio, sr = k.create("Hello world", voice=style, speed=1.0)
```

**Available voices:** `af_bella`, `af_heart`, `af_alloy`, `af_aoede`, `af_sarah`, `af_sky`, `am_adam`, `am_michael`
**Default voice:** `af_bella`
**Speed:** 1.0 (normal), 0.4 (fast with time-stretch)

**Critical quirks:**
- **Method is `k.create()`, NOT `k.generate()`** — `generate()` does not exist
- **Voice embedding as `voice` parameter**, not `style` — pass the numpy array directly as `voice=style`
- Voice embedding shape is `(510, 1, 256)` — collapse with `mean(axis=0)` which gives `(1, 256)` (already 2D, no extra reshape needed)
- ONNX model expects `input_ids` as `int64`, `speed` as `float32`
- Audio output is `(1, N)` — must flatten to `(N,)`
- 510-phoneme context limit (~400 chars per segment)

## Pipeline

```
PLAN --> CODE --> RENDER --> STITCH --> KOKORO AUDIO --> REVIEW
```
...
5. **KOKORO AUDIO** — Generate narration per scene with Kokoro TTS, concat, then mux with video
...
class Scene1_Introduction(Scene):
    def construct(self):
        ...
        self.play(FadeIn(title), run_time=1.5)  # NOT Write() — Text is not a vectorized mobject
        self.wait(1.0)
...
- **`self.camera.background_color`** set in `setup()`, not `construct()` (Manim 0.21+)
- **Clean exits** — FadeOut all mobjects at scene end: `self.play(FadeOut(Group(*self.mobjects)))`
- **Use `FadeIn()` for Text and Code objects** — `Write()` only works on vectorized mobjects (shapes, paths). `Text` and `Code` are not vectorized.
- **For inline code display, use `Text()` not `Code()`** — the `Code` class expects a file path, not inline code. Use `Text('code here', font_size=22, font=MONO)` for inline code snippets.
...
```bash
# Generate per-scene narration with Kokoro
python3 -c "
from kokoro_onnx import Kokoro
k = Kokoro('model.onnx', 'voices-v1.0.bin')
audio, sr = k.create('Your narration text here', voice='af_bella', speed=1.0)
import scipy.io.wavfile as wav
wav.write('scene1.wav', sr, audio.flatten().astype('float32'))
"

# Concat all narration
cat > audio_concat.txt << 'EOF'
file 'scene1.wav'
file 'scene2.wav'
EOF
ffmpeg -y -f concat -safe 0 -i audio_concat.txt -c copy full_narration.wav

# Mux narration with video
ffmpeg -y -i final.mp4 -i full_narration.wav -c:v copy -c:a aac -b:a 192k -shortest final_narrated.mp4
```
...
| `references/manim-021-kokoro-fixes.md` | Manim 0.21 Write/FadeIn, Code class, Kokoro create() API fixes |
