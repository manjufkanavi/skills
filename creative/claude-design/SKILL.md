...
## Multi-Variant Sketching (sketch)

For throwing away 2-3 design variants to compare:

- **When NOT to use sketch:** production components → use claude-design; polished one-off artifact → use claude-design; diagram → use excalidraw or architecture-diagram
- **Variant axes:** pick one axis (density, emphasis, aesthetic, layout, grounding) and diverge from it
- **Interactive bar:** one state transition minimum (filter, toggle, modal) — frozen static images are worse than sloppy animated ones
- **Comparison table:** present variants side-by-side with an opinionated assessment

See the `sketch` reference material in `references/sketch-variants.md` for the full workflow.
...
**Template-Adaptation Pattern** — When the user provides an existing HTML deck template:
1. Read the template's full HTML (CSS, JS, page structure). Also scan for alternate templates in the same directory (e.g., `template_a_minimal.html` vs `template_b_dark.html`).
2. Identify reusable components: `.page` slides, `.header-bar`, `.footer-bar`, corner marks, nav bar, card layouts, stat blocks
3. Replace content with domain-appropriate material
4. Adjust colors/lighting if the template has a light theme and the domain is technical (dark backgrounds for data-heavy reports). Prefer the richer template when the existing one is plain/b&w — a "Template B" style with full color system is better than a "Template A" minimal variant.
5. Verify at least the cover slide with `browser_vision` before committing
6. The template's CSS framework (fonts, responsive scaling, print styles, navigation) should be preserved — only content and data change

**Template Selection Pitfall:** When the existing presentation is "poorly formatted and no colors," do NOT just reflow content into the same template. Find the project's richer alternate template (often `template_b_dark.html` or similar) and build from that. The new presentation may inherit 6+ color variables, gradient definitions, badge system, and layout utilities from the template.

**SVG In Deck Presentations:** When embedding SVG architecture diagrams in HTML slides:
- CSS `var(--*)` variables DO NOT work reliably for SVG `<rect fill>`, `<line stroke>`, or SVG `<marker>` colors in all browsers. Use explicit hex or `rgba()` values in SVG attribute strings.
- For SVG `<text>` elements, `var(--*)` works because text rendering resolves CSS properties.
- When migrating a presentation to a new template, SVG diagrams may need re-styling: assign colored column backgrounds, colored box borders, and color-matched arrow markers.
- Always verify SVG rendering with `browser_vision` on the architecture slide specifically — it often renders broken even when other slides look fine.

Known template collections (load when user references a specific theme or deck):
- `personal_bot/skills/ux_design/html/` — 8 presentation themes (Nordic, Kyoto, Deep Blue, Mint & Sand, Rose & Graphite, Electric Violet, Coral & Sky) with numbered IDs like `7_coral_sky_people_first.html`

**Academic / Research presentations:** When the user is a PhD or M.Tech student building a progress report or mid-semester deck, prefer the project's existing benchmark deck template and follow the `benchmark-construction` skill's `references/midsem-presentation-pattern.md` for the standard slide sequence. This produces consistent visual identity across all project presentations.

**HTML-to-PPTX export:** When the user wants a downloadable `.pptx` from the HTML deck, use the `html-to-pptx-builder` reference pattern: render each slide individually via Playwright (avoid `page-break-after` which Playwright ignores), convert to images via PyMuPDF at 200 DPI, embed in a blank `python-pptx` presentation. See `references/html-to-pptx-builder.md`.
...
**When NOT to adapt a template:** when the design brief calls for a completely different aesthetic, brand system, or interaction model. In that case, start from scratch per the Deck Rules above.
