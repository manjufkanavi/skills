...
metadata:
  hermes:
    ...
    12|    related_skills: [claude-design, popular-web-designs, design-md]
...
### SVG Architecture Diagrams (from architecture-diagram skill)

Generate professional, dark-themed technical architecture diagrams as standalone HTML files with inline SVG graphics.

**When to use:** Software system architecture, cloud infrastructure, microservice topology, database + API maps, deployment diagrams. **When NOT to use:** Physics/chemistry diagrams, hand-drawn whiteboard sketches, floor plans, animated explainers.

**Scope:** Dark, grid-backed tech aesthetic. Best for infrastructure/tech diagrams.
...
**Typography:** JetBrains Mono, 12px names, 9px sublabels, slate-950 background with 40px grid.

**Technical Details:**
- Components: Rounded rects (rx="6") with double-rect masking (opaque bg + semi-transparent styled rect)
- Connections: Draw arrows early (z-order), use SVG markers for arrowheads
- Security flows: Dashed rose lines (#fb7185)
- Boundaries: Dashed rose for security groups (4,4), dashed amber for regions (8,4)
- Spacing: 60px standard height, 40px vertical gap minimum, message buses in gaps
- Legend: CRITICAL placement — outside all boundary boxes, 20px below lowest boundary

**Output:** Single self-contained .html file, no external dependencies, no JavaScript.

**Template:** See `templates/architecture-diagram.html` for the full working HTML template with all component types.
