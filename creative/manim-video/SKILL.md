...
Run `scripts/setup.sh` to verify all dependencies. Requires: Python 3.10+, Manim Community Edition v0.21+ (`pip install manim`), LaTeX (`texlive-full` on Linux, `mactex` on macOS), and ffmpeg. Reference docs tested against Manim CE v0.21.0. See `references/troubleshooting.md` for 0.21 compatibility gotchas (`Number` removed, `Arrow` needs `Create()`, `dash_ratio` gone, `DOWN_LEFT` → `DOWN + LEFT`, background in `setup()`).
