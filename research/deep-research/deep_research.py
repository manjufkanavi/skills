...
def _extract_themes(items):
    ...
    for t, c, u in items:
        combined = (t + " " + c).lower()
        if any(k in combined for k in ["what is", "defined as", "means", "introduction", "overview", "paradigm", "concept"]):
            themes["definition"].append((t, c, u))
        if any(k in combined for k in ["evolution", "history", "shift", "transition", "from.*to", "paradigm shift", "prompt engineering.*loop engineering"]):
            themes["evolution"].append((t, c, u))
        if any(k in combined for k in ["how it works", "architecture", "mechanism", "loop structure", "agent loop", "feedback", "iterate", "plan", "observe", "verify", "system design", "framework"]):
            themes["mechanics"].append((t, c, u))
        if any(k in combined for k in ["challenge", "limitation", "problem", "criticism", "difficulty", "barrier", "risk", "adapting", "migration", "transition", "full code", "codebase", "production", "failure", "cost", "token", "context window", "fails", "miss", "dependency", "overhead"]):
            themes["challenges"].append((t, c, u))
        if any(k in combined for k in ["use case", "application", "example", "case study", "industry", "enterprise", "workflow", "practice", "claude code", "cursor", "coding", "developer", "engineering team", "tool"]):
            themes["applications"].append((t, c, u))
        if any(k in combined for k in ["future", "outlook", "next", "trend", "evolving", "roadmap", "2026", "emerging", "will", "expect", "prediction"]):
            themes["future"].append((t, c, u))
        if any(k in combined for k in ["expert", "research", "academic", "study", "paper", "scholar", "survey", "analysis", "abstract", "university", "anthropic", "openai", "karpathy", "boris"]):
...
def _build_thematic_report(topic, all_items, ref_offset=1):
    ...
    sections = []
    if themes["definition"]:
        sections.append(_synthesize_section(
            "1. Definition and Conceptual Overview",
            themes["definition"], ref_offset=ref_offset
        ))
    if themes["evolution"]:
        sections.append(_synthesize_section(
            "2. Evolution: The Paradigm Shift from Prompt Engineering to Loop Engineering",
            themes["evolution"], ref_offset=ref_offset
        ))
    if themes["mechanics"]:
        sections.append(_synthesize_section(
            "3. Technical Mechanics and System Architecture",
            themes["mechanics"], ref_offset=ref_offset
        ))
    if themes["challenges"]:
        sections.append(_synthesize_section(
            "4. Challenges and Limitations in Production Adoption",
            themes["challenges"], ref_offset=ref_offset
        ))
    if themes["applications"]:
        sections.append(_synthesize_section(
            "5. Applications and Real-World Use Cases",
            themes["applications"], ref_offset=ref_offset
        ))
    if themes["expert_views"]:
        sections.append(_synthesize_section(
            "6. Expert Perspectives and Academic Research",
            themes["expert_views"], ref_offset=ref_offset
        ))
    if themes["future"]:
        sections.append(_synthesize_section(
            "7. Future Outlook and Emerging Trends",
            themes["future"], ref_offset=ref_offset
        ))
    if not sections:
        sections.append(_synthesize_section(
            "Analysis",
            deduped, ref_offset=ref_offset
        ))
    return sections

def _build_concise_report(topic, all_items, ref_offset=1, all_data=None):
    """Build a tight, readable report (~5-10 pages, 8K chars max)."""
    if not all_items:
        return None
    deduped = _deduplicate_content(all_items)
    if not deduped:
        return None
    # Pick top 3 sources per theme
    themes = _extract_themes(deduped)
    ...
    for theme_key in theme_order:
        ...
        if not items:
            continue
        # Take top 3 longest (most informative) items
    ...
    lines.append("---")
    lines.append("")
    lines.append(f"*Report generated by Deep Research Skill v2. Content deduplicated and synthesized thematically. Max 8K chars for concise reading.*")
    # Enforce hard limit
    full = "\n".join(lines)
    if len(full) > MAX_REPORT_LENGTH:
        # Trim references to fit
        ref_start = full.find("## References")
        if ref_start > 0:
            full = full[:ref_start].rstrip() + "\n\n---\n\n*Report generated by Deep Research Skill v2. Content deduplicated and synthesized thematically. Max 8K chars for concise reading.*"
    return full
...
def _build_slide_deck(topic, all_items, ref_offset=1, all_data=None):
    ...
    Uses the same theme extraction and content selection as the markdown
    report so the slideshow and report contain the same data.
    """
    if not all_items:
        return None
    deduped = _deduplicate_content(all_items)
    ...
    for theme_key in theme_order:
        ...
        for title, content, url in items_sorted:
            clean = _clean_text(content)
            # Extract 1-2 key sentences (same logic as report synthesis)
            sentences = re.split(r'(?<=[.!?])\s+', clean)
            for sent in sentences:
                sent = sent.strip()
...
def generate_report(topic, all_data):
    ...
    # Preserve a per-run timestamped raw copy so the fixed research_data.json
    # (overwritten each run) never destroys a previous topic's raw data.
    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    raw_copy_path = os.path.join(os.path.dirname(data_path), f"research_data-{ts}.json")
    with open(raw_copy_path, "w") as f:
        json.dump(payload, f, indent=2)

    print(f"  Structured research data written to: {data_path}")
    print(f"  Per-run raw copy saved to: {raw_copy_path}")
...
RESEARCH_DATA_PATH = os.path.join(
    "/Users/manjunathkanavi/.hermes/skills/research/deep-research",
    "research_data.json",
)

def publish_to_deep_research(topic, timestamp, raw_data_path, md_report_path=None, slide_deck=None):
    """Publish one research run to the deep-research repo in a fixed format.
    ...
    Returns the commit message on success, or None if publishing was skipped/failed.
    """
    try:
        os.makedirs(DEEP_RESEARCH_REPO, exist_ok=True)
    except OSError as e:
        print(f"  ⚠ Could not ensure deep-research repo dir exists: {e}")
        return None

    safe_topic = re.sub(r'[^a-z0-9\-]', '-', topic.lower().replace(' ', '-'))
    slug = safe_topic

    # 1. raw_data/ + json/ — copy the per-run payload verbatim
    try:
        os.makedirs(os.path.join(DEEP_RESEARCH_REPO, "raw_data"), exist_ok=True)
        os.makedirs(os.path.join(DEEP_RESEARCH_REPO, "json"), exist_ok=True)
        raw_name = f"{slug}-{timestamp}.json"
        for cat in ("raw_data", "json"):
            with open(raw_data_path, "rb") as src_f, \
                    open(os.path.join(DEEP_RESEARCH_REPO, cat, raw_name), "wb") as dst_f:
    ...
    # 3. commit + push the deep-research repo
    try:
        subprocess.run(["git", "add", "-A"], cwd=DEEP_RESEARCH_REPO, check=True, capture_output=True)
        subprocess.run(
            ["git", "commit", "-m", f"Deep research: {topic}"],
            cwd=DEEP_RESEARCH_REPO, check=True, capture_output=True,
        )
        subprocess.run(["git", "push"], cwd=DEEP_RESEARCH_REPO, check=True, capture_output=True)
        print(f"  Published + pushed to {DEEP_RESEARCH_REPO} (raw_data, json, markdown, html)")
        return f"Deep research: {topic}"
    except subprocess.CalledProcessError as e:
        print(f"  ⚠ Deep-research repo commit/push failed: {e}")
        return None
...
def run_research(topic):
    ...
    broad_queries = [
        f"{topic}", f"{topic} overview", f"{topic} introduction", f"{topic} definition",
        f"{topic} explained", f"{topic} examples", f"{topic} applications", f"{topic} use cases",
        f"{topic} benefits", f"{topic} challenges", f"{topic} limitations", f"{topic} future",
        f"{topic} trends", f"{topic} research", f"{topic} academic", f"{topic} survey",
        f"{topic} review", f"{topic} analysis", f"{topic} comparison", f"{topic} vs",
        f"{topic} case study", f"{topic} best practices", f"{topic} implementation",
    ...
    deep_queries = [
        ...
        f"{topic} lessons learned", f"{topic} real world examples", f"{topic} industry adoption",
        f"{topic} enterprise use", f"{topic} production deployment", f"{topic} scalability",
        f"{topic} performance", f"{topic} evaluation", f"{topic} metrics", f"{topic} benchmark",
        f"{topic} comparison study", f"{topic} systematic review", f"{topic} literature review",
        f"{topic} state of the art", f"{topic} recent advances", f"{topic} open problems",
        f"{topic} open challenges", f"{topic} open research questions", f"{topic} open issues",
    ...
    # NOTE: raw_data/ + json/ were already published above (before the early
    # return). The markdown/html summaries are published by this skill's agent
    # workflow after synthesis, via publish_to_deep_research(..., md_report_path=...).
